...
public final class RagPrompts {
    ...
    public static final String DEFAULT_SYSTEM_PROMPT = """
            ...
            3. 모든 문장 끝에 근거로 사용한 컨텍스트 번호를 반드시 붙입니다. 빠뜨리면 안 됩니다.
               인용 형식은 [1] 또는 [1][2]처럼 씁니다.
            4. 질문이 요구한 내용을 먼저 직접 답하고, "모두", "항목별"이라는 요구가 있으면 근거에 있는 항목을 빠짐없이 나열합니다.
            5. 문서에 있는 표현을 최대한 그대로 사용하고, 불필요한 예시나 질문과 무관한 문장은 만들지 않습니다.
            """;
     ...
     *
     * <p>벡터 검색 뒤에 키워드 결과를 단순 추가하면 최종 topK가 이미 벡터 결과로 가득 차서
     * 키워드 검색이 실제 답변에 전혀 반영되지 않는다. RRF는 서로 다른 점수 체계를 직접 비교하지 않고
     * 각 목록에서의 순위만 사용하므로 벡터 유사도와 키워드 점수를 안전하게 섞을 수 있다.
     */
