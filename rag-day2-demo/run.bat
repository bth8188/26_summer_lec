...
rem        run.bat chatbot                (제주·김치 CLI 챗봇 — 먼저 docker compose up -d 필요)
rem        run.bat api                    (같은 챗봇을 REST로 — http://localhost:8080/swagger-ui.html)
...
if "%~1"=="chatbot" call mvnw.cmd spring-boot:run -Dspring-boot.run.profiles=%~1 & goto :eof
if "%~1"=="api" call mvnw.cmd spring-boot:run -Dspring-boot.run.profiles=%~1 & goto :eof

echo 사용법: run.bat ^<chunking-strategies^|lab21^|lab22^|lab23^|chatbot^|api^>
exit /b 1
